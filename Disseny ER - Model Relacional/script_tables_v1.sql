...
CREATE SCHEMA IF NOT EXISTS hospital_blanes;

SET search_path TO hospital_blanes;

CREATE TABLE usuaris (
    id SERIAL PRIMARY KEY,
    usuario VARCHAR(50) UNIQUE NOT NULL,
    password_hash TEXT NOT NULL,
    nom_rol VARCHAR(50) NOT NULL CHECK (nom_rol IN ('Administratiu', 'Metge', 'Infermer', 'Personal Vari')),
...
CREATE TABLE tipus_aparell (
    id_tipus_aparell SERIAL PRIMARY KEY,
    nom_tipus_aparell VARCHAR(100) NOT NULL UNIQUE
);
...
CREATE TABLE aparell (
    ...
    descripcio TEXT,
    id_tipus_aparell INTEGER,
    CONSTRAINT fk_tipus_aparell FOREIGN KEY (id_tipus_aparell) REFERENCES tipus_aparell(id_tipus_aparell)
);
